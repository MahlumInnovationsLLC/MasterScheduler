      ...
      <div className="flex flex-row flex-1 h-full">
        ...
        <div className="bay-schedule-viewport flex-grow overflow-auto" ref={viewportRef}>
          ...
          <div className="manufacturing-bays mt-2">
            {/* Generate array of elements for teams with weekly headers */}
            {bayTeams
              .filter(team => {
              ...
              .flatMap((team, teamIndex) => {
                // Create array of elements to return
                const elements = [];
                
                // Add weekly header row after every other team (odd index)
                if (teamIndex % 2 === 1 && teamIndex > 0) {
                  elements.push(
                    <div 
                      key={`header-after-team-${teamIndex}`}
                      className="week-header-row mb-1 flex bg-gray-900"
                      style={{
                        width: `${Math.max(10000, differenceInDays(new Date(2030, 11, 31), dateRange.start) * (viewMode === 'day' ? slotWidth : slotWidth / 7))}px`,
                      }}
                    >
